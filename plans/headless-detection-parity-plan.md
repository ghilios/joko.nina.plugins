...
- [x] Add a shared helper (in `DiagnosticUtil`) that returns an `IRenderedImage`:
...
- [x] Load with `isBayered:` the frame's real value (the `_BayeredN_` filename token, as live does) instead of
      today's hard-coded `false` (`DiagnosticUtil.cs:91-93`). That flag only sets `Properties.IsBayered`; pixel
      data is untouched.
- [x] **Bayer-pattern precedence must match live** (`ImageControlVM.cs:611-621`): profile
...
- [x] **Honour `profileService.ActiveProfile.ImageSettings.DebayerImage`** — live gates the whole debayer on it
...
- [x] Promote `HocusFocusSplitFrameDetector` from `private sealed` to `internal sealed`
...
- [x] **Measure memory on the largest bank run before committing.** `IDebayeredImage` holds the raw `ushort[]`
...
- [x] `GoldenEvalRunner.cs:178` — highest value, it scores recall/precision
- [x] `RecommendRunner.cs:172`, `DiagnoseLabelsRunner.cs:293`, `AfFitDiagnosticRunner.cs:144`,
...
- [x] `InspectAlignRunner.cs:122` — **was missing from the original list**; drives the real
      `SensorModel.RegisterStarsAndFit`
- [x] `StarReview/StarReviewRunner.cs:160` — **was missing**; labels authored here feed the optimizer
- [x] `ContaminationDiagnosticRunner.cs:172` — shared path for `.xisf`/`.fits`; **keep `.tif` on the Mat route**
      (NINA's TIFF loader normalizes by `1<<16` vs DiagnosticUtil's `ushort.MaxValue`, a ~1.5e-5 shift, and a TIFF
      has no CFA anyway)
- [x] `GoldenRunner.cs:124` and `AnnotateRunner.cs:92` do **not** detect — they render tiles/overlays. They should
...
- [x] `OptimizationDiagnosticRunner` (~`:461`) and `BankVerifyRunner.cs:181`: put the `IDebayeredImage` in
...
- [x] Delete `MatSplitFrameDetector` and `HarnessDetection.ToFrameDetectionResult` for these paths — they are the
      mirror being removed.
- [x] **`HotpixelThreshold` / `HotpixelThresholdingEnabled` must remain live searched axes.** The probe proved a
...
- [x] Fix `RunFitConfig.PreferredModel`: `afOptions.HyperbolicFitModel` in the loader
      (`RunEvaluationLoader.cs:226`) but hard-coded `null` in `OptimizationDiagnosticRunner.cs:453` and
      `BankVerifyRunner.cs:208`.

## Task A — `TiltCalibrationRunner` (MISSING FROM THIS PLAN; found during Task 3)

The plan scheduled Tasks 2–3 by call site and missed the tilt harness, which was both the last runner detecting
on a raw mosaic and the last holder of the two mirrors — so `MatSplitFrameDetector` and
`HarnessDetection.ToFrameDetectionResult` survived Task 3 for it alone. Its `--debayer` opt-in did not close the
gap either: it debayered at load time WITHOUT the CFA filter (spec row 2, the configuration that leaves the
optimum floored). This is the runner `DiagnosticUtil`'s own warning was written about.

- [x] All three detection paths (optimizer, 4-corner regions, per-star paraboloid) load `LoadRenderedImage` and
      detect through the plugin's `HocusFocusSplitFrameDetector` / `Detect(IRenderedImage, …)`.
- [x] `--debayer` **removed** (accepted-and-warned for one release, not hard-rejected: the behaviour it asked for
      is now what happens unconditionally, so failing the run would be gratuitous).
- [x] Both mirrors deleted with the file that held them; `grep` confirms no remaining reference.

## Task 4 — The two other defects (see spec)

- [x] **`ExportLinearRunner.cs:69` — debayer, do NOT CFA-filter.** Its own doc already claims it debayers; the
...
- [x] **`StarDetectionOptimizerWizardVM.LoadFloatMatFromDisk` (`:4152-4171`) — a live-app bug.** It hard-codes
...
- [x] **Mono byte-identity.** The strongest guard: a mono bank run's optimizer output must be unchanged before vs
      after. 18 of 22 bank runs are mono.
- [x] **Bayered parity.** Pin the measured target: `bobp` lands `Sensitivity = 10`, min stars ≈ 29.
- [x] **Hot-pixel axes still bite** — mutate the threshold, assert detection output changes.
- [x] **Guard against recurrence, properly.** After the refactor, **delete** `LoadFloatMat`'s
...
- [x] Full plugin suite green. `SendAsync_WritesOnABackgroundThread` is known-flaky and unrelated.

## Task 6 — Record what is now stale

- [x] Note in the spec which stored artifacts the fix invalidates: goldens for `SorenVance`, `bobp`, `bobp_m101`,
      `timmer`; their `optimized_settings.json`; existing `verification_*.md`; and the `bobp_m101` recall analysis.
- [x] Add the parity requirement to `.claude/docs/testapp-cli.md`.
- [x] Note the pre-existing per-filter gap: with per-filter star detection enabled, live resolves the target
