...
- **`AberrationSurface.cs:117`'s `tiltAmountMicrons < 0` guard** — the same NaN-permeable shape Task 6 fixed elsewhere (`< 0` and `<= 0` both admit NaN), but on a value that is genuinely *not* shielded, unlike the focal length. `request.TiltAmountMicrons` ← `options.TiltAmountMicrons`, whose setter performs no heal and which has no downstream guard, so a NaN would reach the aberration surface and produce an all-NaN frame with no error. Two routes in: NaN persisted in the options store, or a NaN `AberrationDelta` via `SimulatedTiltAdapterVM.ApplyDelta` (`Math.Min(NaN, bound)` returns NaN, which is then stored unfiltered). Note that the usual "NaN can't be in the store" reasoning is **false** for this class — `simScrew4AngleDegrees` is deliberately persisted as `double.NaN` (the 3-screw convention), so the store demonstrably carries NaN. Deliberately not fixed in Task 6: unlike the focal-length case (demonstrated by a failing test — `new DefocusModel(double.NaN, ...)` threw nothing), this route is reasoned rather than proven, and Task 6's guard conversion was scoped to the optics chain actually fed by the new profile-inference path. Fixing it means converting the guard *and* deciding the heal/validation policy for `TiltAmountMicrons` and `ApplyDelta`, which is a design question, not a two-line change.
