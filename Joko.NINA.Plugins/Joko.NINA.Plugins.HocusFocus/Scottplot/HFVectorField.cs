...
namespace NINA.Joko.Plugins.HocusFocus.Scottplot {
    ...
    public class HFVectorField : IPlottable {
        ...
        private readonly HFArrowStyle ArrowStyle = new HFArrowStyle();
        ...
        public float LineWidth { get => ArrowStyle.LineWidth; set => ArrowStyle.LineWidth = value; }
        ...
        public HFVectorField(
            ...
            for (int i = 0; i < xs.Length; i++) {
                for (int j = 0; j < ys.Length; j++) {
                    if (colormap != null) {
                        var vectorLength = vectors[i, j].Length();
                        if (vectorLength >= colorScaleMax) {
                            intensities[i, j] = 1.0;
                        } else if (vectorLength <= colorScaleMin) {
                            intensities[i, j] = 0.0;
                        } else {
                            intensities[i, j] = (vectorLength - colorScaleMin) / (colorScaleMax - colorScaleMin);
                        }
                    }
        ...
        public void ValidateData(bool deep = false) {
            /* validation occurs in constructor */
        }
