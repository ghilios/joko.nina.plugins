    ...
    public class HocusFocusVM : BaseVM, IAutoFocusVM {
        ...
        public void SetCurveFittings(string method, string fitting) {
            ...
            if (LastAutoFocusPoint?.Timestamp != lastGeneratedReportTimestamp) {
                // The loaded chart is not the run this VM generated (foreign run, or another VM's report), so the
                // initial-position / Start-HFR / HFR-change rows still describe the PREVIOUS live run. Re-populate
                // them from the LOADED run's own report — which carries all three — and fall back to the sentinels
                // (collapsed rows) whenever it cannot be found or does not carry the value. Leaving the previous
                // run's numbers on screen is the 2026-07-29 field report; showing 0 as though it were a measurement
