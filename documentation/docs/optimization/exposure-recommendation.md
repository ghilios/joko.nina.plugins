...
When the Star Detection Optimizer's search lands **Brightness Sensitivity** at the very bottom of its
range, the wizard's Summary page shows a **Star signal** block above the recommended step size. The gate
having to sit that low means the detector accepted almost anything above the noise to find stars at all:
the frames are signal-starved, not merely permissively gated, and the focus curve fitted from them rests
on low-confidence detections. The fix the block offers is a longer exposure, not a lower gate, and it
derives the number from the same accepted-star measurements the run already made.

The logic lives in `ExposureRecommender.cs`; its Summary copy lives in `StarSignalCopy.cs`. Both are pure
and static, so every state described below is unit-tested without a running wizard.
...
The threshold is 1.0, not exactly 0. Sensitivity's search axis spans `[0, 50]` with an initial step of
1.0, and the pattern search that lands it near the floor refines that step by repeated halving down to
one eighth of it, so a genuinely floored search routinely settles on 0.125, 0.25, or 0.5 rather than
bit-exact 0; testing for exactly 0 would miss most real cases. 1.0 is one full search step off the floor,
and also exactly a tenth of the default gate this recommendation targets. 2.0 was considered and
rejected: a legitimately hand-tuned gate of 2 on a rich, well-exposed field is plausible on its own and
should not trigger an exposure nag.
...
For each frame in the run, skipping the far-from-focus recovery wing (deliberately sparse, and not
representative of ordinary gate strength), the recommender reduces every accepted star's
sensitivity-gate SNR down to one number: the **N-th brightest** surviving star, where N is the same
star-count target the objective's own median-count score uses (\(N_{\text{target}}\), 20 by default; 60
when the run optimized for aberration inspection). A frame with fewer than N surviving stars falls back
to its faintest survivor instead of guessing, an under-estimate rather than an invented number, and is
counted separately as a **short frame**.
...
statistic is pinned to the objective's own star-count knee rather than a literal number baked into the
recommender: retuning the objective's target moves this recommendation with it, instead of the two
...
- **At most 4× the current exposure in one step.** Under \(\text{SNR} \propto \sqrt{t}\), a 4× time
  factor is exactly a 2× SNR jump: a statement about how far one exposure change can be trusted, not a
  taste choice. A signal-starved run is usually starved by more than that, so the cap deliberately leaves
  the number short and lets a second optimizer pass, now working from frames with real signal, refine the
  answer, rather than committing to one large, unverified extrapolation.
...
current exposure; an unchanged value is never rounded. Because that last step always rounds up, the
number you see can land one ladder step past whichever cap applied. Treat the caps as the intent behind
the number, not a hard 30 s ceiling on the digits themselves.
...
fainter candidates below your genuinely bright stars. If several frames also found fewer stars than the
objective's star-count target, the block names that too: a star-poor field the low gate is scraping to
fill, not necessarily something a longer exposure would fix.

## Live vs. saved runs

What the block asks you to *do* about the number depends on where the run's frames came from.

A **Live sweep** puts an editable exposure box beside the block, pre-filled with the recommended value,
and a **Capture a new sweep and optimize** button: adjust the number if you want, then the wizard
recaptures the sweep at that exposure and re-tunes detection settings against the new frames, without
leaving the wizard.

A **saved ("Replay") run** has no rig attached to recapture from, so the block instead tells you to raise
your auto-focus exposure to about the recommended value in NINA's focuser options and run the wizard
again in Live mode once you have frames at the new exposure.

Either way, **accepting the run in front of you is still a legitimate choice**: the settings it found are
the best fit for the frames you actually have, low-confidence detections or not.
...
!!! note "Detection binning changes the same measurement"
    ...
    re-measure the exposure, rather than stacking both changes at once.
...
- [Detection Binning](../settings/detection-binning.md): the sibling recommendation on the same Summary
  page, also derived from a measurement rather than an estimate.
