...
| **Tilt Astigmatism** | 0.25 | How much of the tilt carries the *corrector* with it rather than tilting the sensor alone, as a fraction. A sensor cannot change the beam in front of it, so sensor-only tilt merely reveals the corner astigmatism above — and its effect fades as the tilt grows, until an extremely tilted corner renders round again and can still be focused to a point. A sagging focuser or a non-square thread tilts the corrector too, which adds a split proportional to the tilt: elongation that is **radial on every edge** and that holds an axis ratio of (1+f)/(1−f) however far the tilt is pushed, with a corner that can never be focused sharp. 0 models a camera simply sitting crooked in a square adapter. |
