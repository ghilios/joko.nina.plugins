...
| **F76** | **NEW, product defect (wave 21).** The optimizer wizard's whole action row — `Back / Review frames / Continue optimizing / Accept / Close` — is **clipped off the bottom** on a long summary: the window is `SizeToContent`, WPF clamps it to the work area, and **window height == work-area height == 1392, exactly**. All five buttons sit in the visual tree at `(0,0)`. No ScrollViewer, no resize. **180 passing VM tests cannot see it** — the failing half is the XAML. Fix: ScrollViewer around the content with the action row pinned outside |
...
| **the suite** | **3839**, verified **by COUNT** at the end of wave 21 (`Failed: 0, Passed: 3839, Skipped: 0`). **"3781", "3831" and "3838" are ALL stale.** Verify by count, never the tick ([F37](followups.md)) |
