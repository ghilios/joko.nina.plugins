...
| **F63(b)** | Pin the SHIPPED default rather than `astrodet`'s in the harness. **After wave 14 they are the same value**, so this is now a cleanup rather than a coordinate-system move — check that before pricing it as one | ~1 h + re-derivation | ~1 h |
