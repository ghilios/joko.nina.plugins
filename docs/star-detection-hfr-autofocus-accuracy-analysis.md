...
| 5. Small fixes (F7–F14) | ✅ Done (PR #51, merged) | One PR: F9 (AddOffset now carries PeakBrightness/contamination flag/translated background plane), F10 (ResetDefaults sets Simple_FocusRange via property + StarPeakResponse 0.6→0.75), F13 (kappa-sigma masks zero pixels from iteration 0), plus weight-chain follow-ups W1 (SolveHuberIrls keeps last good solution on mid-loop failure), W2 (Huber comparison centered on residual median), F12 (parabolic Grubbs now weighted), W3 (SensorModel per-star fits routed through WeightRegularization). F14 document-only (saturated cores unmasked in HFR — XML doc on MeasureStar). F7/F8 won't-fix by decision (rarely-used paths). **F11 pulled from this batch during execution**: the meanFlux denominator fix (`/numUnclippedPixels` instead of `/starPoints.Count`) is correct but dropped accepted stars 8.1% on the corpus image (1970→1810) — the BrightnessSensitivity knob was tuned against the inflated NormalizedBrightness, so it needs a recalibration pass (→ step 6, mirrors the F4 σ-consistency precedent). Plans: `small-fixes-f7-f14-{design,plan}.md`. |
| 6. F11 meanFlux fix (no recalibration needed) | ✅ Done (PR #52) | meanFlux now divides by the clip-survivor count (`numUnclippedPixels`) for an honest peak-vs-survivor-mean NormalizedBrightness; `MeanBrightness` keeps its full-footprint denominator by design (brightest-N AF path, F8). **No BrightnessSensitivity recalibration**: real-data focus-sweeps (anchor + 4 AF runs, measured per preset via a new `--noise-level` diagnostic) show the fix is behavior-preserving at every shipped preset (None/Low/Typical/High) — the gate is non-binding at the Low/Typical 2.0 default and the footprint-vs-survivor gap is small at the presets' StructureLayers=4. The −8.1% anchor drop (1970→1810) reproduces only at the *advanced* profile's operating point (BrightnessSensitivity=4, StructureLayers=6), not at 2.0 as originally assumed; advanced users covered by a release note. Tests: gate-flip direction pin + default-yield non-regression guard. Plans: `f11-meanflux-sensitivity-recalibration-{design,plan}.md`; results: `f11-meanflux-sensitivity-recalibration-results.md`. |
| 7. Structural IRLS robustness (W4) | ⬜ Not started | Judge Huber residuals against an unweighted reference fit so a high-weight displaced point cannot self-mask (recovery cliff at ≥ ~1.25× capped weight ratio — see `weight-chain-hygiene-design.md` §1 implementation finding). Changes every weighted Huber fit (AF curves, Hybrid selection, LOO, SensorModel per-star); needs FitQualityRunner corpus validation before any change. |
